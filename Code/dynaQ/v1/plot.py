...
def plot_transition_probabilities(ax, world, border=None, **kwargs):
	...
	Args:
		...
		`pyplot.tripcolor`.
	"""
	xy = [(x - 0.5, y - 0.5) for y, x in product(range(world.full_size + 1), range(world.full_size + 1))]
	xy += [(x, y) for y, x in product(range(world.full_size), range(world.full_size))]

	t, v = [], []
	for sy, sx in product(range(world.full_size), range(world.full_size)):
		...
		# compute cell points
		bl, br = sy * (world.full_size + 1) + sx, sy * (world.full_size + 1) + sx + 1
		tl, tr = (sy + 1) * (world.full_size + 1) + sx, (sy + 1) * (world.full_size + 1) + sx + 1
		cc = (world.full_size + 1)**2 + sy * world.full_size + sx
	...
	ax.set_aspect('equal')
	ax.set_xticks(range(world.full_size))
	ax.set_yticks(range(world.full_size))
	ax.set_xlim(-0.5, world.full_size - 0.5)
	ax.set_ylim(-0.5, world.full_size - 0.5)
...
def plot_state_values(ax, world, values, border, **kwargs):
	...
	if is_square(world.n_states):
		p = ax.imshow(np.reshape(values, (world.full_size, world.full_size)), origin='lower', **kwargs)
	else:
		p = ax.imshow(np.reshape(values, (1, world.full_size)), origin='lower', **kwargs)

	if border is not None:
		if is_square(world.n_states):
			for i in range(0, world.full_size + 1):
				ax.plot([i - 0.5, i - 0.5], [-0.5, world.full_size - 0.5], **border, label=None)
				ax.plot([-0.5, world.full_size - 0.5], [i - 0.5, i - 0.5], **border, label=None)
				for x in range(world.full_size):
					for y in range(world.full_size):
						plt.text(x,y, '%.2f' % values[x+world.full_size*y],
			...
			for i in range(0, world.full_size + 1):
				ax.set_yticks([])
				ax.plot([-0.5, world.full_size - 0.5], [1 - 0.5, 1 - 0.5], **border, label=None)
				for x in range(world.full_size):
...
def plot_stochastic_policy(ax, world, policy, border=None, **kwargs):
	...
	if is_square(world.n_states):
		...
		for sy, sx in product(range(world.full_size), range(world.full_size)):
			# print(sy, sx)
			state = world.coordinate_to_state((sx, sy))
		...
		xy = [(x - 0.5, 1 - 0.5) for x in range(world.full_size + 1)]
		# print(xy)
		xy += [(x, 1) for x in range(world.full_size)]
		# print(xy)

		t, v = [], []
		for sx in range(world.full_size):
			# print(sy, sx)
			state = world.coordinate_to_state(sx)

			# compute cell points
			bl, br = 1 * (world.full_size + 1) + sx, 1 * (world.full_size + 1) + sx + 1
			tl, tr = (1 + 1) * (world.full_size + 1) + sx, (1 + 1) * (world.full_size + 1) + sx + 1
			cc = (world.full_size + 1)**2 + 1 * world.full_size + sx
