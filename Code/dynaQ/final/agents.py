	...
	def plan(self, irl_output):
		for n in range(self.N):
			...
			if irl_output is not None:
				# print(irl_output.reshape((5,5)))
				r =irl_output[new_s]+self.world.rewards[new_s]
				lr =np.square(r)
				if new_s == s:
					r-=1
				# print(s, new_s, act, np.square(r))
				# print((self.r_model[s][act]/self.count[s][act]), irl_output[new_s])
				# print(np.exp(r))
			else:
				r = (self.r_model[s][act]/self.count[s][act])
			self.update_values(s, act, r, new_s, 0.1)

	def update_values(self, old_state, action, reward, new_state, lr):
		cur_q = self.q_array[old_state][action]
		max_future_q = np.max(self.q_array[new_state])
		new_q = cur_q + lr*(reward + self.gamma*max_future_q - cur_q)
		...
		for eps in range(episodes):
			...
			while not done:
				...
				if new_s == s:
					reward-=1
				# update values
				self.update_values(s, action, reward, new_s, 0.1)
