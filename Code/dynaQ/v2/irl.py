...
def m_irl(world, trajectories, lr):
...
def local_action_probability(world, rewards):
	...
	er = np.multiply(np.exp(rewards),np.eye((n_states)))
	...
	for _ in range(2 * n_states):
		for a in range(n_actions):
			za[:,a] = np.matmul(er, np.matmul(p_transition[:,:,a], zs.T))/3
			# print(za)
			# za = np.array([er * p[a].dot(zs) for a in range(n_actions)]/np.array([3])).T
		zs = za.sum(axis=1)
	return za / zs[:, None]
