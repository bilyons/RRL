...
def main():
	...
	t_list = add_trajectories(world, agent, 10)

	# Do IRL
	# fe = I.feature_expectations_addable(world, t_list, feat_count, 10)
	# p_i = I.inital_probabilities_addable(world, t_list, init_count, 10)
	rewards = I.m_irl(world, t_list, 20000, 0.1)
	# Reflect my boy
	# transform = np.zeros((25))
	# transform[world.terminals] = 1.0
	r_dif = world.rewards - rewards

	# print(rewards.reshape((5,5)))
	# r_dif = r_dif*transform
