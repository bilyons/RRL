...
def plot_state_values(ax, world, values, border, **kwargs):
	...
	Args:
		...
		`pyplot.imshow`.
	"""
	if is_square(world.state_space):
		p = ax.imshow(np.reshape(values, (world.size, world.size)), origin='lower', **kwargs)
	else:
		p = ax.imshow(np.reshape(values, (1, world.size)), origin='lower', **kwargs)
	...
	if border is not None:
		if is_square(world.state_space):
			...
			for i in range(0, world.size + 1):
				ax.set_yticks([])
				ax.plot([-0.5, world.size - 0.5], [1 - 0.5, 1 - 0.5], **border, label=None)
...
def plot_stochastic_policy(ax, world, policy, border=None, **kwargs):
	...
	if is_square(world.state_space):
		...
		xy = [(x - 0.5, 1 - 0.5) for x in range(world.size + 1)]
		# print(xy)
		xy += [(x, 1) for x in range(world.size)]
		# print(xy)

		t, v = [], []
		for sx in range(world.size):
			# print(sy, sx)
			state = world.coordinate_to_state(sx)

			# compute cell points
			bl, br = 1 * (world.size + 1) + sx, 1 * (world.size + 1) + sx + 1
			tl, tr = (1 + 1) * (world.size + 1) + sx, (1 + 1) * (world.size + 1) + sx + 1
			cc = (world.size + 1)**2 + 1 * world.size + sx
