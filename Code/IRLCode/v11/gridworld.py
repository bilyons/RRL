...
class LineWorld:
	...
	def __init__(self, size, p_slip, terminals, rewards):
		self.size = size
		...
		self.dif = self.rewards[self.state_space-1] - self.rewards[self.state_space - self.size]

		self.p_slip = p_slip
		# Determine trasition probabilities
		self.transition_prob = self._transition_prob_table()
		self.p_transition = [np.array(self.transition_prob[:,:,a]) for a in range(self.action_space)]
...
class GridWorld:
	...
	def reflexive_reward(self, state, maxent_array, rewards):
		diff = np.abs(np.abs(maxent_array[self.state_space-self.size] - maxent_array[self.state_space-1]) - self.dif)
		if state == self.state_space-1:
			proportion = self.rewards[state]/(self.rewards[state] - self.dif)
			# If small reward bigger and at small, no reflexive
			if maxent_array[state] < proportion*maxent_array[self.state_space-self.size]:
				# big reward too small
				return diff
			else:
				# Big reward correct or larger
				return -diff
		elif state == self.state_space-self.size:
			proportion = self.rewards[state]/(self.rewards[self.state_space -1])
			# If small reward bigger and at small, no reflexive
			if maxent_array[state] < proportion*maxent_array[self.state_space-1]:
				# big reward too small
				return diff
			else:
				# Big reward correct or larger
			...
			print("You can't terminate here")
			print("error in logic")
			exit()

		# diff = np.abs(np.abs(maxent_array[0] - maxent_array[self.state_space-1]) - self.dif)
		# if state == self.state_space-1:
		# 	proportion = self.rewards[state]/(self.rewards[state] - self.dif)
		# 	# If small reward bigger and at small, no reflexive
		# 	if maxent_array[state] < proportion*maxent_array[0]:
		# 		# big reward too small
		# 		return diff
		# 	else:
		# 		# Big reward correct or larger
