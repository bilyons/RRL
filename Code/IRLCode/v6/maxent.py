...
def irl(world, features, terminals, trajectory, optim, init, feat_count, init_count, ep_number, eps=1e-4, eps_esvf=1e-5):
	...
	while delta > eps:
		...
		# compute gradient
		e_svf = compute_expected_svf(world, p_initial, terminals, reward, eps_esvf)
...
def expected_svf_from_policy(world, p_initial, terminals, p_action, eps=1e-5):
...
def local_action_probabilities(world, terminals, reward):
	"""
...
def compute_expected_svf(world, p_initial, terminals, reward, eps=1e-5):
	"""
	...
	"""
	p_action = local_action_probabilities(world, terminals, reward)
	return expected_svf_from_policy(world, p_initial, terminals, p_action, eps)
